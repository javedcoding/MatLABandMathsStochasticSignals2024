...
Fk = [1 0 dt 0; 0 1 0 dt; 0 0 1 0; 0 0 0 1] ; %State Transition Matrix 
Bk = [dt^2/2; 
    dt^2/2; 
    dt; 
    dt]; %Input Control Matrix
% Hk = [0 0 1 0; 0 0 0 1]; % Measurement matrix for position and then velocity
Hk = [1 0 0 0; 0 0 1 0]; % Measurement matrix for position and then velocity
 
% main variables
u = 1.5; % acceleration mag
x= [0; 0; 0; 0]; %initial state vector, car has two components: [x;y;xdot;ydot]
...
G = [dt^4/4; dt^3/2; dt^3/2; dt^2];
sigmaw = G * G' * car_accel_noise_mag^2; % Process noise covariance matrix
Rk = [10 0; 0 10];% measurement noise covariance matrix
...
for t = 0 : dt: duration
    ...
    processNoise = car_accel_noise_mag * [(dt^2/2)*randn; (dt^2/2)*randn; dt*randn; dt*randn];
...
for t = 1:length(pos)
    ...
    vel_estimate = [vel_estimate; xhat(3)];  %velocity using kalman filter only
